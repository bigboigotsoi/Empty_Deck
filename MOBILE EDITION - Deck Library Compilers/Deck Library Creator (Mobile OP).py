...
class Deck_Library:
    self.parent = "My Decks"

Intro = False
# PyIDLE = False
Tabbing = True
SpeedType = 1   # 1 Slowest, 3 fastest
...
ContinuingAnswer = lambda line: any(line) and not (ItsA("Question", line, BulletFor)
...
def Start(Intro):
    ...
    if Intro:
        JC.SmartPace("\t'Converts deck Files in the " + deckFolder + " folder into a 'deck Library' format.")
...
def Array_Arrange(Folder):
    Arranged_Array = []
    Folder += '\\'
    Is_Folder = lambda thing: os.path.isdir(Folder + thing)
    Is_File = lambda thing: not Is_Folder(thing)
    sorted_directories = \
        sorted(list(filter(Is_Folder, os.listdir(Folder)))) + sorted(list(filter(Is_File, os.listdir(Folder))))

    for directory in sorted_directories:
        if Is_Folder(directory):
            Arranged_Array.append({directory: Array_Arrange(Folder + directory)})
        else:
            Arranged_Array.append(directory)
    return Arranged_Array

def WhichDeck(deckFolder):

    JC.TitledList("Would you like to...", ["Convert the entire deck folder into the deck library?",
                                           "Convert one deck file to a deck library field"], 1)
    if JC.GetInput('', 1) == '1':
        FolderFileNames = Array_Arrange(deckFolder)
    else:
        deckNumber = 0
        print("\n / Pre-loaded Decks:")
        JC.Pause(2)
...
def LearnBullets(LearningBullets):
    if LearningBullets:
        ...
        while BulletFor["Questions"] == BulletFor["Answers"]:
            ...
            BulletFor["Questions"], BulletFor["Answers"], BulletFor["Comments"] = (
...
def ReadAndVerified(fileName, FolderFileNames, BulletFor):
    ...
    DeckFile = list(filter(lambda Line: any(JC.Pure(Line)) and
                                        not ItsA("Comment", Line, BulletFor), JC.TryToRead(DeckFile)))

    # Detect file/bullet errors...
    JC.ErrorIf(JC.OneArrayed([BulletFor["Questions"], BulletFor["Answers"]], DeckFile),
        ...
        for fileName in FolderFileNames:
            ...
            while QuestionsLeft:
                ...
                DeckArray += Comma_After(Quote(
                    re.sub(BulletFor["Questions"], '', DeckFile[lineNumber]))) + New_Level(' ')
                ...
                while AddingAnswers:
                    ...
                    # Lambdas for clarity...
                    AnswerWillGrow = ContinuingAnswer(JC.TheItem("After", thisLine, DeckFile))

                    CompleteAnswer = ItsA("Answer", thisLine, BulletFor) and not \
                        ContinuingAnswer(JC.TheItem("After", thisLine, DeckFile))

                    # RE-FORMAT AND ADD ANSWERS:
                    if CompleteAnswer:
                        # It's already complete, so just add with quotes.
                        DeckArray += Quote(FixSpaces(re.sub(BulletFor["Answers"], '', thisLine)))

                        # If another answer is next, add a comma.
                        if ItsA("Answer", JC.TheItem("After", thisLine, DeckFile), BulletFor):
                            DeckArray = Comma_After(DeckArray)

                    # Deal with growing, multi-line answers...
                    elif AnswerWillGrow or ContinuingAnswer(thisLine):
                        # Grow it more...
                        GrowingAnswer += re.sub(BulletFor["Answers"], '', thisLine)
                        GrowingAnswer = FixSpaces(GrowingAnswer)

                        # When the answer is finished growing...
                        if not ContinuingAnswer(JC.TheItem("After", thisLine, DeckFile)):
                            # Add it as the one, long, grown answer.
                            DeckArray += Quote(GrowingAnswer)
                            GrowingAnswer = ''
                            if ItsA("New Answer", JC.TheItem("After", thisLine, DeckFile), BulletFor):
                                DeckArray = Comma_After(DeckArray)

                    # If the last answer for the question, or complete last answer...
                    if ItsA("Question Coming Next", JC.TheItem("After", thisLine, DeckFile), BulletFor) \
                ...
                if JC.TheItem("Is the Last Line of the File", thisLine, DeckFile):
                    DeckArray += ']'
                    if not JC.TheItem("Is the Last File in the deck Folder", fileName, FolderFileNames):
                        DeckArray = Comma_After(DeckArray)
                    elif ConvertingAll:
                        DeckArray += '}'
            ...
            if SpeedType <= 2:
                ...
                print("\n\t* Completed Conversion! * ~"
                      + str(round(100*(FolderFileNames.index(fileName) + 1)/len(FolderFileNames))) + "%")
                JC.Pause(2)
                print()
